...
An automated Data Engineering pipeline deployed on **Kubernetes (Kind)** using **Apache Airflow 3** and **Google Cloud Platform (GCP)**. It fetches real-time air pollution data for Kraków (PM10, PM2.5, AQI) from Open-Meteo API, stores raw JSON in **Google Cloud Storage (Data Lake)**, transforms the data using **Python**, and loads relational analytical tables into **Google BigQuery (Data Warehouse)**.
